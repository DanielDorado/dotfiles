...
;; As eglot depends on project-find-go-mod and this search for .git and Go when installing
;; does not copy .git then we change it to use go.mod instead.
(defun my/project-find-go-mod (dir)
  "Find the deepest directory with a go.mod file starting from DIR upward."
  (let ((current-dir (expand-file-name dir))
        (deepest nil))
    (while (not (equal current-dir "/"))
      (when (file-exists-p (expand-file-name "go.mod" current-dir))
        (setq deepest current-dir))
      (setq current-dir (file-name-directory (directory-file-name current-dir))))
    (when deepest
      (cons 'transient (file-name-as-directory deepest)))))

(add-hook 'project-find-functions #'my/project-find-go-mod)
#+END_SRC
